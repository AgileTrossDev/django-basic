# *****************************************************************************
# Original code from https://github.com/Stefan-Korner/SpacePyLibrary          *
# DG local mods for formatting PEP8 and cleanup. Added the block parity func  *
#                                                                             *
...
# each with 7 data bytes and 1 check byte
#
# +---+---+---+---+---+---+---+---+
...
#
SHIFT_REGISTER_STATE_TRANSITIONS = []
...
#############
# -----------------------------------------------------------------------------
def generate_shift_register_values():
    """generates the values of the galois field"""
    global SHIFT_REGISTER_STATE_TRANSITIONS
    for sreg_state in range(0, 128):
        transition_field = []
        for value in range(0, 256):
            sreg = sreg_state  # handle the next shift register state
            mask = 0x80
            while mask != 0:
            ...
            transition_field.append(sreg)
        SHIFT_REGISTER_STATE_TRANSITIONS.append(transition_field)


# -----------------------------------------------------------------------------
def encode_step(sreg, value):
    """performs an incremental step in the BCH encoding: 1,...,7 """
    global SHIFT_REGISTER_STATE_TRANSITIONS
    return SHIFT_REGISTER_STATE_TRANSITIONS[sreg][value]


# -----------------------------------------------------------------------------
def encode_stop(sreg):
...
def block_parity(byte_block):
    """ Take up to seven byte block of data and calculate the parity using above.
        Should handle codeblock length 40bits, 48bits, and 56bits
    """
    sreg = 0
    for b in byte_block:
        sreg = encode_step(sreg, b)

    return encode_stop(sreg)
...
generate_shift_register_values()
